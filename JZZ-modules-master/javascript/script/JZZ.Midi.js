...
JZZ_.MIDI=function(){
 var tmp=function(){};
 tmp.prototype=JZZ_.Midi.prototype;
 var obj=new tmp();
 JZZ_.Midi.apply(obj,arguments);
 return obj;
}
JZZ_.Midi=function(){
 var d='';
 if(arguments.length==1){
  var arg=arguments[0];
  if(arg instanceof JZZ_.Midi){
   d=arg.data();
  }
  else if(typeof arg =='string'){
   for(var i=0;i<arg.length;i++) if(arg.charCodeAt(i)>255) throw new Error("JZZ_.Midi: Bad input parameters");
   d=arg;
  }
  else if(arg instanceof Array){
   for(var i=0;i<arg.length;i++){
    var x=parseInt(arg[i]);
    if(isNaN(x)||!isFinite(x)||x<0||x>255) throw new Error("JZZ_.Midi: Bad input parameters");
 ...
 else throw new Error("JZZ_.Midi: Bad input parameters");
 this.data=(function(x){ return function(){ return x;}})(d);
}
JZZ_.Midi.len=function(x){
...
for(var i=0;i<128;i++){
 ...
 JZZ_.Midi[k]=i; JZZ_.MIDI[k]=i;
 k=[,'Db',,'Eb',,,'Gb',,'Ab',,'Bb'][n];
 if(k){ k+=o; JZZ_.Midi[k]=i; JZZ_.MIDI[k]=i;}
}

JZZ_.Midi.prototype.hex=function(){
 var s=this.data();
 var a=[];
...
JZZ_.Midi.prototype.array=function(){
 var s=this.data();
 var a=[];
 for(var i=0;i<s.length;i++) a.push(s.charCodeAt(i));
 return a;
}
JZZ_.Midi.prototype.toString=function(){
 var str=this.hex();
