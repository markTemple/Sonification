...
JZZ_.MidiFile=function(){
 var type=1;
 var ppqn=96;
 var fps;
 var ppf;
 if(arguments.length==1){
  if(arguments[0] instanceof JZZ_.MidiFile){
...
JZZ_.MidiFile.prototype=(function(p){ function f(){}; f.prototype=p; return new f();})(Array.prototype);
JZZ_.MidiFile.prototype.dup=function(x){
 throw new Error('Copy-constructor not yet implemented');
}
JZZ_.MidiFile.prototype.load=function(s){
...
JZZ_.MidiFile.prototype.loadSMF=function(s){
 ...
 while(p<s.length){
  ...
  this.push(new JZZ_.MidiFile.Chunk(type,data));
  p+=len;
 }
 if(p!=s.length || n!=this.ntrk) throw new Error("Corrupted MIDI file");;
}
JZZ_.MidiFile.prototype.dump=function(){
 var s=''; var k=0;
 for(var i=0;i<this.length;i++){
  if(this[i] instanceof JZZ_.MidiFile.MTrk) k++;
  s+=this[i].dump();
 }
...
JZZ_.MidiFile.prototype.toString=function(){ return "Midi File "+this.length};
JZZ_.MidiFile.prototype.var2num=function(s){
...
JZZ_.MidiFile.prototype.msglen=function(n){
 switch(n&0xf0){
...
JZZ_.MidiFile.Chunk=function(t,d){
 if(this.sub[t]) return this.sub[t](t,d);
 if(typeof t!='string' || t.length!=4) throw new Error("Invalid chunk type: "+t);
 this.type=t;
 this.data=d;
};
JZZ_.MidiFile.Chunk.prototype=(function(p){ function f(){}; f.prototype=p; return new f();})(Array.prototype);
JZZ_.MidiFile.Chunk.prototype.sub={
 'MThd':function(){ throw new Error("Illegal chunk type: MThd");},
 'MTrk':function(t,d){ return new JZZ_.MidiFile.MTrk(d);}
};
JZZ_.MidiFile.Chunk.prototype.dump=function(){
...
JZZ_.MidiFile.MTrk=function(s){
 if(s==undefined){
  this.push(new JZZ_.MidiFile.Event(0,'\xff\x2f',''));
  return;
 }
 var t=0;
 var p=0;
 var w='';
 while(p<s.length){
  var d=JZZ_.MidiFile.prototype.var2num(s.substr(p,4));
  p++; if(d>0x7f) p++; if(d>0x3fff) p++; if(d>0x1fffff) p++;
  t+=d;
  if(s.charCodeAt(p)==0xff){
   var st=s.substr(p,2); p+=2;
   var m=JZZ_.MidiFile.prototype.var2num(s.substr(p,4));
   p++; if(m>0x7f) p++; if(m>0x3fff) p++; if(m>0x1fffff) p++;
   this.push(new JZZ_.MidiFile.Event(t,st,s.substr(p,m)));
   p+=m;
  }
   ...
   w=s.substr(p,1); p+=1;
   var m=JZZ_.MidiFile.prototype.msglen(w.charCodeAt(0));
   this.push(new JZZ_.MidiFile.Event(t,w,s.substr(p,m)));
   p+=m;
  }
...
JZZ_.MidiFile.MTrk.prototype.dump=function(){
 var s='';
 var t=0;
 var m='';
 for(var i=0;i<this.length;i++){
...
JZZ_.MidiFile.MTrk.prototype.getTime=function(){ return this[this.length-1].time;}
JZZ_.MidiFile.MTrk.prototype.setTime=function(t){
 t=parseInt(t);
...
JZZ_.MidiFile.MTrk.prototype.eventOrder=function(s,d){
...
JZZ_.MidiFile.MTrk.prototype.addEvent=function(t,s,d){
 ...
 this.splice(i,0,new JZZ_.MidiFile.Event(t,s,d));
}
JZZ_.MidiFile.MTrk.prototype.addMidi=function(){
 var t=arguments[0];
 var args=[];
...
JZZ_.MidiFile.MTrk.prototype.addNote=function(t,ch,note,vel,dur){
 var n=parseInt(note);
...
JZZ_.MidiFile.MTrk.prototype.addName=function(t,str){ this.addEvent(t,'\xff\x03',str);}
JZZ_.MidiFile.MTrk.prototype.addText=function(t,str){ this.addEvent(t,'\xff\x01',str);}
JZZ_.MidiFile.MTrk.prototype.addTempo=function(t,bpm){
...
JZZ_.MidiFile.Event=function(t,s,d){
 this.time=t;
 this.status=s;
 this.data=d;
};
JZZ_.MidiFile.Event.prototype.toString=function(){
 var str;
...
JZZ_.MidiFile.Player=function(){
 this.tracks=0;
 this.playing=false;
 this.looped=false;
};
JZZ_.MidiFile.Player.prototype=(function(p){ function f(){}; f.prototype=p; return new f();})(Array.prototype);
JZZ_.MidiFile.Player.prototype.loop=function(b){ this.looped=b?true:false;}
JZZ_.MidiFile.Player.prototype.play=function(){
...
JZZ_.MidiFile.Player.prototype.stop=function(){
 this.event='stop'; this.paused=undefined;
 if(this.playing) this.playing=false;
 else this.onEvent({control:this.event});
}
JZZ_.MidiFile.Player.prototype.pause=function(){
 this.event='pause'; this.playing=false;
}
JZZ_.MidiFile.Player.prototype.resume=function(){
...
JZZ_.MidiFile.Player.prototype.tick=function(){
 var t=(new Date()).getTime();
...
JZZ_.MidiFile.b64="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
JZZ_.MidiFile.fromBase64=function(input){
 var output='';
 var chr1,chr2,chr3;
 ...
 while(i<input.length){
  enc1=JZZ_.MidiFile.b64.indexOf(input.charAt(i++));
  enc2=JZZ_.MidiFile.b64.indexOf(input.charAt(i++));
  enc3=JZZ_.MidiFile.b64.indexOf(input.charAt(i++));
  enc4=JZZ_.MidiFile.b64.indexOf(input.charAt(i++));
  chr1=(enc1<<2)|(enc2>>4);
...
JZZ_.MidiFile.toBase64=function(data){
 ...
 do{
  ...
  tmp_arr[ac++]=JZZ_.MidiFile.b64.charAt(h1)+JZZ_.MidiFile.b64.charAt(h2)+JZZ_.MidiFile.b64.charAt(h3)+JZZ_.MidiFile.b64.charAt(h4);
 } while(i<data.length);
 enc=tmp_arr.join('');
 var r=data.length%3;
