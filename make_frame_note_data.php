...
foreach($codon_plus_array2 as $rf_numb => $array){
	...
	foreach($array as $c_numb => $codon_data_arrays){
		...
		//BOTH of these are unreliable for motif notes BOTH WRONG
//		$frame_note_data["$rf_numb"]["$c_numb"]['PLAY NOTE from $c_numb'] 		
//		= ($rf_numb*160)+($c_numb*$this2next);
//		$frame_note_data["$rf_numb"]["$c_numb"]['PLAY NOTE from $dna_seq_numb'] 
//		= (($rf_numb*160)+(($dna_seq_numb-$rf_numb)/$len_bp2sonify)*$this2next);
		
//		$frame_note_data["$rf_numb"]["$c_numb"]['pseudo $c_numb rf1'] = (($dna_seq_numb-1)/$len_bp2sonify);
//		$frame_note_data["$rf_numb"]["$c_numb"]['pseudo $c_numb rf2'] = (($dna_seq_numb-2)/$len_bp2sonify);
//		$frame_note_data["$rf_numb"]["$c_numb"]['pseudo $c_numb rf3'] = (($dna_seq_numb-3)/$len_bp2sonify);

		//this formula will no work for motifs
		...
		$frame_note_data["$rf_numb"]["$c_numb"]['calculated c_numb'] = 	$theCalc_c_numb;
		$frame_note_data["$rf_numb"]["$c_numb"]['calculated rf'] = 		$theCalc_rf;
										
		//BOTH OF THESE ARE GOOD to play motifs in proper place	(Actully both are the same calculation)							
		$frame_note_data["$rf_numb"]["$c_numb"]['PLAY NOTE CALCULATED'] 
		= (($theCalc_rf*$timeBetweenFrames)+(($dna_seq_numb-$theCalc_rf)/$len_bp2sonify)*$this2next);
		//= ($theCalc_rf*160)+($theCalc_c_numb*$this2next);

		$rf_numb = $remember_this_rf_numb;
		//$this_note = (($theCalc_rf*160)+(($dna_seq_numb-$theCalc_rf)/$len_bp2sonify)*$this2next);
		$calc_note = (($theCalc_rf*$timeBetweenFrames)+(($dna_seq_numb-$theCalc_rf)/$len_bp2sonify)*$this2next);
		//var_dump($this_note);
		$this_note = intval($calc_note);		
                                //$this_note = $this_note + 500;
...
//		$frame_note_data["$rf_numb"]["$c_numb"]['advance'] = 		$advance;
//		$frame_note_data["$rf_numb"]["$c_numb"]['motif_bp'] = 		$motif_bp;
//		$frame_note_data["$rf_numb"]["$c_numb"]['motif_seq'] = 		$motif_seq;
//		$frame_note_data["$rf_numb"]["$c_numb"]['motif_name'] = 	$motif_name;
		...
		// new attributes
		$frame_note_data["$rf_numb"]["$c_numb"]['textCol'] = 'black';
		$frame_note_data["$rf_numb"]["$c_numb"]['tempo'] = $tempo;
//;
		
		$frame_note_data["$rf_numb"]["$c_numb"]['aa_formated'] = '<font color="black">'.$aa.'</font>'.'|';
		$frame_note_data["$rf_numb"]["$c_numb"]['protein_note_formated'] = '<font color="black">'.$note_dot.'</font>'.'|';	
...
// adjust time based on codon eg G= =5 C==5 A= -5 T =-5		
		...
		$frame_note_data["$rf_numb"]["$c_numb"]['codon_formated'] = '<font color="black">'.$codon.'</font>'.'|';
		
		$frame_note_data["$rf_numb"]["$c_numb"]['1bp_formated'] = '<font color="black">'.$codon.'</font>'.' |';
		
		$frame_note_data["$rf_numb"]["$c_numb"]['note_formated'] = 	'<font color="black">'.$note_dot.'</font>'.'|';	
		
// lower volume for stops and raise vol for starts	
...
//echo $_POST['stopstart'];
		...
		elseif( ($_POST['stopstart'] == 'strict' or $_POST['stopstart'] == 'silent')
		
		
		and ($_POST['frameNum'] == '3') and ($rf_numb !== 0) ){
			...
			if($rf_numb !== 0){
				...
				$track_header['motif_string']["$c_numb"] .= '<font color="black">'.$codon_data_arrays['note_1chr'].'</font>';
			}
		}
	}
}
